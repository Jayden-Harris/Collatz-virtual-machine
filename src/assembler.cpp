...
std::vector<int> assemble_file_to_instructions(const std::string& file_name, VMstate& vmstate) {
  ...
  // --- First Pass: Read all lines and record labels with their instruction address ---
  while (true) {
      std::string line;
  ...
  // --- Second Pass: Convert tokens to integers, replacing labels with addresses ---
  ...
  for (const auto& line : lines) {
      ...
      // Special handling for MOV reg, reg -> emit MOV_REG opcode instead
      if (!tokens.empty() && tokens[0] == "MOV" && tokens.size() == 3) {
          if (is_register(tokens[1]) && is_register(tokens[2])) {
              ...
              continue;  // skip normal token processing for this line
          }
      }

      // Normal processing for other instructions or MOV with immediate
